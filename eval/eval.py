...
def icl_lm_eval(
    ...
    if 'llama3.1' in model_name.lower():
        ...
        target_ids = tokenizer(target, return_tensors='pt')['input_ids'].to(device)
        encodings = tokenizer(f' [INST] {x} [/INST] {target}', return_tensors='pt')   
        input_ids = encodings['input_ids'].to(device)
        attention_mask = encodings['attention_mask'].to(device)
        logits = model(input_ids=input_ids, attention_mask=attention_mask).logits
...
def main(model_name, model_true_name, data_path, save_path, source, target):
    ...
    if 'sft' in model_name:
        directory = f'./eval/result/sft/'
    else:
        directory = f'./eval/result/orpo/'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", default="./eval/data/bizsre_test.json", help="Path to the input JSON file.")
